...
class Piper:
    ...
    def send_action(self, action):
        ...
        """
        # SO101 -> Piper 映射
        #val = action["shoulder_lift.pos"]
        # 添加 motor_dir
        motor_dir = [-1, 1, -1, -1, -1, -1]  # 对应关节 1~6 的方向修正
        val = action["shoulder_lift.pos"] * math.pi / 180
        val *= motor_dir[1]  # 乘上方向系数，第2关节
        print(f"[DEBUG] shoulder_lift.pos raw value = {val}\n--------")


        # 限位 (根据你之前给的数据，大概范围)
        so101_min, so101_max = -1.67, 0
        piper_min, piper_max = 0, 1.57
