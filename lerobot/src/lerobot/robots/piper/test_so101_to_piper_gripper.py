...
class Piper:
    ...
    def send_action(self, action):
        ...
        val *= gripper_dir[0]  # 乘上方向系数，第一关节
        print(f"[DEBUG] gripper.pos raw value = {val}\n--------")


        # 限位 (根据你之前给的数据，大概范围)
        so101_min, so101_max = 0, 1.67
        piper_min, piper_max =0, 100
        ...
        # 转换为整数单位 (Piper SDK GripperCtrl 接受 int mm * 1000)
    ...
    # 调试打印
        print(f"[Piper][Gripper] 目标开度={projected_val_mm:.3f} mm (SO101Leader原值={val:.3f})")
