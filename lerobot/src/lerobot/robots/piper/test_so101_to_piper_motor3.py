...
class Piper:
    ...
    def send_action(self, action):
        ...
        val = action["elbow_flex.pos"] * math.pi / 180
        val *= motor_dir[2]  # 乘上方向系数，第一关节
        print(f"[DEBUG] elbow_flex.pos raw value = {val}\n--------")


        # 限位 (根据你之前给的数据，大概范围)
        so101_min, so101_max = -1.67, 1.67
        piper_min, piper_max =-2.9 , 0
