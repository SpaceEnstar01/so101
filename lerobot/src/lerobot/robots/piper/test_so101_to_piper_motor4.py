...
'wrist_roll.pos':  ,  -> motor_6
'gripper.pos':   -> gripper

'''

'''
but it may be better choice
...
class Piper:
    ...
    def send_action(self, action):
        ...
        val = action["wrist_roll.pos"] * math.pi / 180
        val *= motor_dir[5]  # 乘上方向系数，第一关节
        print(f"[DEBUG] wrist_roll.pos raw value = {val}\n--------")


        # 限位 (根据你之前给的数据，大概范围)
        so101_min, so101_max = -1.5, 1.5
        piper_min, piper_max =-1.8, 1.8
