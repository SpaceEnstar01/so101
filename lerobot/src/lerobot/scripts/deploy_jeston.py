...
def record(cfg: RecordConfig) -> LeRobotDataset:
    ...
    # ============ 新增代码开始：自动生成唯一数据集名 ========== Add by zexuan ==========
    import uuid
    from datetime import datetime
    
    # 只对local/eval_开头的数据集名进行自动扩展
    if cfg.dataset.repo_id.startswith("local/eval_"):
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        unique_id = str(uuid.uuid4())[:8]  # 取UUID前8位作为唯一标识
        auto_repo_id = f"{cfg.dataset.repo_id}_{timestamp}_{unique_id}"
        logging.info(f"自动生成数据集名: {auto_repo_id}")
    else:
        auto_repo_id = cfg.dataset.repo_id
    # ============ 新增代码结束：自动生成唯一数据集名 ========== Add by zexuan ==========

    if cfg.resume:
        dataset = LeRobotDataset(
            #cfg.dataset.repo_id,
            # ============ 修改代码开始：使用自动生成的数据集名 ========== Add by zexuan ==========
            auto_repo_id,  # 修改这里
            # ============ 修改代码结束：使用自动生成的数据集名 ========== Add by zexuan ==========
        ...
        # ============ 修改代码开始：使用自动生成的数据集名 ========== Add by zexuan ==========
        sanity_check_dataset_name(auto_repo_id, cfg.policy)  # 修改这里
