...
class BiSO101Leader(Teleoperator):
    ...
    This bimanual leader arm wraps two SO101Leader instances for dual-arm teleoperation.
    """
    ...
    def __init__(self, config: BiSO101LeaderConfig):
        ...
        # Use left_id if provided, otherwise use config.id with "_left" suffix
        left_id = config.left_id if config.left_id is not None else (f"{config.id}_left" if config.id else None)

        # Use right_id if provided, otherwise use config.id with "_right" suffix
        right_id = config.right_id if config.right_id is not None else (f"{config.id}_right" if config.id else None)
